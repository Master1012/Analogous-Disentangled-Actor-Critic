...
import time
import spinup.sac.core as core
...
class SAC:
    def __init__(self,args,device="cuda"):
        self.env_fn = lambda : gym.make(args.env_name)
        ...
        self.logger_kwargs=setup_logger_kwargs(args.env_name, self.seed)
        self.save_freq=1
        self.device=device
    ...
    def sac(self):
        ...
        ac = self.actor_critic(env.observation_space, env.action_space, **self.ac_kwargs)
        ac=ac.to(self.device)
        ac_targ = deepcopy(ac)
        ac_targ=ac_targ.to(self.device)
        ...
        # Experience buffer
        replay_buffer = ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=self.replay_size)
        ...
        def get_action(o, deterministic=False):
            return ac.act(torch.as_tensor(o, dtype=torch.float32).to(self.device), 
                        deterministic)
